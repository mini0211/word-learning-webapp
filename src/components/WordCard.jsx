export default function WordCard({ word, flipped, onFlip }) {
  ...
  return (
    <button
      type="button"
      onClick={onFlip}
      className="card-3d group h-80 w-full text-left outline-none focus-visible:ring-4 focus-visible:ring-indigo-200"
      aria-label={flipped ? '단어 카드 앞면 보기' : '단어 카드 뜻 보기'}
    >
      <div
        className={`card-inner relative h-full w-full rounded-[2rem] transition-transform duration-500 ${
      ...
      >
        <section className="card-face absolute inset-0 flex h-full w-full flex-col justify-between rounded-[2rem] border border-indigo-100 bg-white p-8 shadow-xl shadow-indigo-100/60">
          ...
          <div>
            ...
            <h2 className="mt-4 break-words text-5xl font-black tracking-tight text-slate-950 sm:text-6xl">
              {word.word}
            </h2>
            {word.reading && <p className="mt-4 text-xl text-slate-500">{word.reading}</p>}
          </div>
          <p className="text-sm text-slate-400">카드를 누르면 뒤집힙니다</p>
        </section>
        ...
        <section className="card-face card-back absolute inset-0 flex h-full w-full flex-col justify-between rounded-[2rem] border border-emerald-100 bg-gradient-to-br from-emerald-500 to-teal-600 p-8 text-white shadow-xl shadow-emerald-100/80">
          ...
          <div>
            ...
            {word.example && (
              <p className="mt-6 rounded-2xl bg-white/15 p-4 text-base leading-7 text-white/90">{word.example}</p>
            )}
          </div>
